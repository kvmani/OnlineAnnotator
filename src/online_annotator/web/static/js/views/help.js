...
export const SHORTCUTS = [
  ["Tools", [
    ...
    ["W", "Magic wand"], ["T", "Box threshold"], ["R", "Polygon threshold"], ["G", "Fill"],
  ]],
  ["Drawing", [
    ["1 … 9", "Select class 1 to 9"], ["Shift + draw", "Erase with any tool"], ["Alt + click", "Pick the class under the cursor"],
    ["[  /  ]", "Smaller / larger: brush and eraser size, wand tolerance, or the previewed threshold"],
    ["Shift + [  /  ]", "Big step: halve / double the brush, tolerance or threshold by 10"],
    ["Enter", "Close a polygon outline / apply a threshold preview"], ["Backspace", "Remove the last polygon corner"],
    ["Esc", "Cancel a polygon outline or threshold preview"],
  ]],
  ["View", [
...
const SECTIONS = [
  ...
  },
  {
    ...
    body: () => [
      ...
      toolRow("brush", "Brush (B)", "Paint the selected class. Type the diameter in pixels on the right, drag its slider, or press [ and ] (Shift+[ and ] halve or double it). A 1 px brush changes exactly one pixel. Zoom in for edges; paint big areas with a large brush."),
      toolRow("eraser", "Eraser (E)", "Turn pixels back into background. It has the same size as the brush. With any other tool, holding Shift erases too."),
      ...
      toolRow("polythreshold", "Polygon threshold (R)", "Like the box threshold, but you click the corners of any shape around the region (close it with a double-click, Enter or the first corner). Only the pixels inside the outline decide the automatic threshold and can be labelled, so a nearby scale bar, grain boundary or differently lit area no longer pulls the threshold off, and a slanted platelet can be followed without taking in a box full of matrix. Keep some matrix inside the outline: traced too tightly, the region is mostly feature and the automatic value splits the feature (the panel warns you). A feature crossing the outline is cut along it."),
      ...
      h("h3", {}, "Useful extras"),
      UL(h("span", {}, B("Exact values"), ": every slider in Tool options has a number box; type a value and press ", K("Enter"), ". ", K("["), " and ", K("]"), " change the current tool's value without leaving the image."),
        h("span", {}, B("Protect other classes"), ": painting then only changes unlabelled pixels, so you cannot damage a neighbouring class."),
  ...
  },
  {
    ...
    body: () => [
      ...
      faq("The magic wand fills far too much.", "Lower the tolerance, zoom in and click a darker part of the feature, or use the box or polygon threshold instead."),
      faq("The polygon threshold breaks a feature into pieces or takes almost everything.", "The outline is probably drawn tightly around the feature, so there is hardly any background for the automatic threshold to compare with. Press Esc and draw the outline with some surrounding matrix inside, or move the threshold by hand ([ and ])."),
